...
def extract_financial_data(text):
    ...
    if cash_balance_match:
        data['cash_balance'] = float(cash_balance_match.group(1).replace(',', ''))
    else:
        st.warning("Couldn't find Cash Balance in the document.")
        data['cash_balance'] = 0
    ...
    if total_profits_match:
        data['total_profits'] = float(total_profits_match.group(1).replace(',', ''))
    else:
        st.warning("Couldn't find Total Profits in the document.")
